    ...
    def run(self):
        ...
        while True:
            ...
            if data['info'] == 'state' and data['position'] == data['action_position']:
                p = random.random()
                if p < 0.5 and 'raise' in data['legal_actions']:
...
if __name__=="__main__":
    ...
    num = 50
    for i in range(num):
        client = Client('TestHalfCR', 'lbr-fc-1-4')
        client.start()
    for i in range(num):
        client = Client('TestHalfCR', 'lbr-fc-3-4')
        client.start()
    for i in range(num):
        client = Client('TestHalfCR', 'lbr-fcpa-1-4')
        client.start()
    for i in range(num):
        client = Client('TestHalfCR', 'lbr-fcpa-3-4')
        client.start()
