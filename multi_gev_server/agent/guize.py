# -*- coding: utf-8 -*-
"""
Created on Thu Dec 19 10:22:53 2019
...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 21 10:44:03 2019
...
def run1_1smallblind(hands):#2轮即可，这个是第一轮
    a=random.random()
    if handsrank1(hands)==9:
        ...
        if a>0.963:
            ...
            bet=200
            action='raise'
    elif handsrank1(hands)==7:#第一轮先手（50）仅存在很差的牌（1，2）才fall牌，否则不弃牌
...
def run1_1bigblind(hands,opobet):#2轮即可，这个是第一轮
    a=random.random()
    if handsrank1(hands)==9:
...
def run1_2smallblind(hands,optbet,selfbet):#2轮即可，这个是第一轮
    a=random.random()
    if handsrank1(hands)>5:#opo bet1*potsize
        action='raise'
        bet=selfbet*2+optbet*2

    elif handsrank1(hands)<6 and handsrank1(hands)>3 :
        if optbet<selfbet*2:
            action='call'
            bet=0
        else:
...
def run1_2bigblind(hands,optbet,selfbet):#2轮即可，这个是第一轮
    a=random.random()
...
def mixactionbig(bet,bet1,bet2,publicbet):#选择动作，通过三个的bet
...
def run2_1bigblind(hands,publichands,publicbet):#2.1大盲位首先动作
    a=random.random()
    handsrank=[]
    publicrank=[]
    handssuit=[]
    ...
    if publicstraight3type=='3threestraight':
        ...
        action2='call'
    # print(bet,bet1,bet2)
    betf=mixactionbig(bet,bet1,bet2,publicbet)
    if betf==0:
        actionf='call'
    else:
        actionf='raise'
...
def run2elseblind(hands,publichands,opobet,publicbetbefore):#2.1小盲位的动作
    ...
    for i in range(3):
        ...
        publicsuit.append(publichands[i][1])
    publicrank3type,publicsuit3type,publicstraight3type=judgepublic3hand(publicrank,publicsuit)#public3type 存在三条，两条，什么都没有；三色同花，两色同花，什么都没有；一个顺子，一个类顺子
    action,bet,ranktype=run2_1bigblind(hands,publichands,publicbetbefore)
    if opobet==0:#对面call牌        
        actionf=action
        betf=bet
        ...
        if bet==opobet:
            if sum(ranktype)>8:
                actionf='raise'
                betf=-publicbetbefore/2+3*opobet
            else:
                actionf='call'
                betf=0
        ...
        if opobet>3000:
            if ranktype[0]>=7 or (ranktype[1]==6) or (ranktype[2]==5):
                actionf='allin'
                betf=20000
            else:
                if a<0.9822:
            ...
            if ranktype[0]>=1.5 or (ranktype[1]==6) or (ranktype[2]==5):
                actionf='raise'
                betf=-publicbetbefore/2+3*opobet
            else:
                actionf='fold'
...
def kind2(n, ranks):#判断有几张是一样的
  for s in ranks:
    if ranks.count(s) == n : return s
  return None#           
def run3_1bigblind(hands,publichands,publicbet):##提示动作，我们
    handsrank=[]
    publicrank=[]
    handssuit=[]
    publicsuit=[]
...
def run3_1smallblind(hands,publichands,opobet,publicbetbefore):#3.1小盲位的动作
    a=random.random()
    handsrank=[]
    publicrank=[]
    handssuit=[]
    ...
    action,bet,rank=run3_1bigblind(hands,publichands,publicbetbefore)
    if opobet==0:#对面call牌
        actionf=action
        betf=bet
    elif opobet==100:
...
def run4_1bigblind(hands,publichands,publicbet):#4.1大盲位动作
    a=random.random()
    handsrank=[]
    publicrank=[]
    handssuit=[]
...
def run4_elseblind(hands,publichands,opobet,publicbetbefore,turn):#3.1小盲位的动作
    a=random.random()
    handsrank=[]
    publicrank=[]
    handssuit=[]
    ...
    action,bet=run4_1bigblind(hands,publichands,publicbetbefore)
    actionf=action
    betf=bet
    if opobet==0:#对面call牌
        actionf=action
        ...
        actionf='fold'
        betf=0
    if opobet>2*publicbetbefore or opobet>2000:
...
def handt(hand):
    ...
    hand1=[]
    for i in range(a):
        hand1.append([turnhandrank(hand[2*i]),turnhandsuit(hand[2*i+1])])
    return hand1
...
def handt2(hand,board):
    ...
    for i in range(a):
        hand1.append([turnhandrank(hand[2*i]),turnhandsuit(hand[2*i+1])])
    for i in range(b):
        hand1.append([turnhandrank(board[2*i]),turnhandsuit(board[2*i+1])])
    return hand1
        
def allturnsmallblindaction(hand,board,opobet,bot_bet,turn):
    a=len(board)
    amount = 0
    if a==0:
        if turn==1:
            advice,amount=run1_1smallblind(handt(hand))
        elif turn==2:
            advice,amount=run1_2smallblind(handt(hand),opobet,bot_bet)
        else:
            if handsrank1(handt(hand))>8:
                advice='allin'
                amount=20000
            else:
                advice='fold'
        ...
        if turn==1:
            advice,amount=run3_1smallblind(handt(hand),handt(board),opobet,bot_bet*2)
        elif turn<4:
        ...
        if turn==1:
            advice,amount=run4_elseblind(handt(hand),handt(board),opobet,bot_bet*2, turn)
        elif turn<4:
            advice,amount=run4_elseblind(handt(hand),handt(board),opobet,bot_bet*2, turn)
        else:
    ...
    return humix(advice,amount)


def allturnbigblindaction(hand,board,opobet,bot_bet,turn):
    a=len(board)
    amount=0
    if a==0:
        if turn==1:
            advice,amount=run1_1bigblind(handt(hand),bot_bet)
        elif turn<4:
            advice,amount=run1_2bigblind(handt(hand),opobet,bot_bet)
        else:
            advice='call'
    elif a==6:
        if turn==1:
            advice,amount,rankty=run2_1bigblind(handt(hand),handt(board),bot_bet*2)
        elif turn<4:
            advice,amount=run2elseblind(handt(hand),handt(board),opobet,bot_bet*2)
        else:
        ...
        if turn==1:
            advice,amount,rankty=run3_1bigblind(handt(hand),handt(board),bot_bet*2)
        elif turn<4:
            advice,amount=run3elseblind(handt(hand),handt(board),opobet,bot_bet*2)
        else:
            advice='call'
        ...
        if turn==1:
            advice,amount=run4_1bigblind(handt(hand),handt(board),bot_bet*2)
        elif turn<4:
            advice,amount=run4_elseblind(handt(hand),handt(board),opobet,bot_bet*2,turn)
        else:
            if handsrank1(handt2(hand,board))>5:
...
# bot_bet=600
# turn=1
# # 6h2d AhKd7h5s4s 0 600 1
# print(allturnsmallblindaction(hand,board,opp_bet,bot_bet,turn))
# print(allturnbigblindaction(hand,board,opp_bet,bot_bet,turn))
# 0 50
# 1 100  ni
# 0 call