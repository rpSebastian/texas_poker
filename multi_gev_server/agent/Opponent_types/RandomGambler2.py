...
hand_strength_band_high = [-100, 100, 500, 500]     # LA, CS, LP, TP
hand_strength_band_low = [-300, -100, -300, 100]

change_step = 50 # 每隔多少局更改一次策略
...
def get_action(data, last_round_raise, hand_strength, hs_band_high, hs_band_low):

    if (hand_strength < hs_band_low) & ('fold' in data['legal_actions']):
        return 'fold'

    elif hand_strength < hs_band_high:
...
if __name__ == "__main__":
    ...
    last_round_raise = 0
    game_num = 0   #游戏局数
    rand_num = random.randint(0, len(hand_strength_band_high)-1)
    hs_band_high, hs_band_low = hand_strength_band_high[rand_num], hand_strength_band_low[rand_num]
    ...
    while True:
        ...
        if data['info'] == 'state':
            ...
            if data['position'] == data['action_position']:
                ...
                action = get_action(data, last_round_raise, lookup.calc(private_card, public_card), hs_band_high, hs_band_low)
                #print(action)
            ...
            last_round_raise = 0
            game_num += 1
            if game_num % change_step ==0:
                rand_num = random.randint(0, len(hand_strength_band_high)-1)
                hs_band_high, hs_band_low = hand_strength_band_high[rand_num], hand_strength_band_low[rand_num]
                print('change', game_num, rand_num)
        else:
            print(data)
            break
    client.close()
